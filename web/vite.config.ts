...
export default defineConfig({
  ...
  worker: {
    ...
    rolldownOptions: {
      output: {
        // The remux, subtitle and JASSUB workers must be able to fetch, so
        // their entries live on a path the plugin-worker CSP does not cover.
        entryFileNames: (chunk) => chunk.name === 'remuxWorker' || chunk.name === 'subtitleWorker'
          || chunk.facadeModuleId?.includes('/jassub/')
