...
export function Carousel({ children, className, prevLabel, nextLabel }: CarouselProps) {
  ...
  return (
    <div className={`carousel ${className ?? ''}`} data-can-prev={canPrev || undefined} data-can-next={canNext || undefined}>
      ...
      {canPrev ? (
        <button type="button" className="carousel-arrow carousel-arrow--prev" aria-label={prevLabel} onClick={() => embla?.scrollPrev()}>
          <ChevronLeft size={28} aria-hidden="true" />
        </button>
      ) : null}
      {canNext ? (
        <button type="button" className="carousel-arrow carousel-arrow--next" aria-label={nextLabel} onClick={() => embla?.scrollNext()}>
